...
class DupontYear:
    def __init__(self, year: int, revenue: float, net_profit: float,
                 ...
                 operating_cash_flow: float = None, total_debt: float = None,
                 cash_equivalents: float = None, operating_profit: float = None,
...
def fetch_us(code: str) -> Optional[DupontResult]:
    ...
    try:
        ...
        cf = ak.stock_financial_us_report_em(stock=code, symbol='现金流量表', indicator='年报')
        ...
        bsp, icp, cfp = pt(bs), pt(inc), pt(cf)
        ...
        for yr in years:
            ...
            ocf = _getv_us(cfp, '经营活动现金流量净额', yr)
            if np.isnan(ocf): ocf = _getv_us(cfp, '经营活动产生的现金流量净额', yr)
            if np.isnan(ocf): ocf = _getv_us(cfp, '经营活动现金净流量', yr)
            cash = _getv_bs_us(bsp, ['现金及现金等价物', '货币资金', '现金', '现金及等价物'], yr)
            std = _getv_bs_us(bsp, ['短期借款', '短期融资'], yr) or 0
            ltd = _getv_bs_us(bsp, ['长期借款', '长期负债'], yr) or 0
            debt = (0 if np.isnan(std) else std) + (0 if np.isnan(ltd) else ltd)
            op = _getv_us(icp, '营业利润', yr)
            da_us = _getv_us(cfp, '折旧及摊销', yr)
            if np.isnan(da_us): da_us = _getv_us(cfp, '折旧与摊销', yr)
            if not any(np.isnan(x) for x in [rev, npv, aa, ae]) and rev > 0 and aa > 0 and ae > 0:
                dy = DupontYear(yr, rev, npv, aa, ae, npv/rev, rev/aa, aa/ae, npv/rev * rev/aa * aa/ae)
                dy.operating_cash_flow = None if np.isnan(ocf) else ocf
                dy.cash_equivalents = None if np.isnan(cash) else cash
                dy.total_debt = None if debt <= 0 else debt
                dy.operating_profit = None if np.isnan(op) else op
                dy.depreciation_amortization = None if np.isnan(da_us) else da_us
                items.append(dy)
...
def fetch_hk(code: str) -> Optional[DupontResult]:
    ...
    try:
        ...
        cf = ak.stock_financial_hk_report_em(stock=code_s, symbol='现金流量表', indicator='年度')
        name = bs['SECURITY_NAME_ABBR'].iloc[0] if not bs.empty else code

        bsp, icp, cfp = _hk_pivot(bs), _hk_pivot(inc), _hk_pivot(cf)
        years = sorted(set(int(c) for c in bsp.columns if str(c).isdigit()) & set(int(c) for c in icp.columns if str(c).isdigit()))
        years = [y for y in years if 2018 <= y <= 2026]
        ...
        for yr in years:
            ...
            aa = (ac + ap) / 2 if not (np.isnan(ac) or np.isnan(ap)) else ac
            ae = (ec + ep) / 2 if not (np.isnan(ec) or np.isnan(ep)) else ec
            ocf = _getv_hk(cfp, ['经营业务现金净额', '经营活动现金流量净额', '经营活动现金流量'], yr)
            cash = _getv_hk(bsp, ['现金及等价物', '现金及现金等价物', '货币资金', '现金'], yr)
            std = _getv_hk(bsp, ['短期贷款', '短期借款', '短期借贷'], yr) or 0
            ltd = _getv_hk(bsp, ['长期贷款', '长期借款', '长期借贷'], yr) or 0
            debt = (0 if np.isnan(std) else std) + (0 if np.isnan(ltd) else ltd)
            op = _getv_hk(icp, ['经营溢利', '营业利润', '经营利润'], yr)
            da_hk = _getv_hk(cfp, ['加:折旧及摊销', '折旧及摊销', '折旧与摊销'], yr)
            if not any(np.isnan(x) for x in [rev, npv, aa, ae]) and rev > 0 and aa > 0 and ae > 0:
                dy = DupontYear(yr, rev, npv, aa, ae, npv/rev, rev/aa, aa/ae, npv/rev * rev/aa * aa/ae)
                dy.operating_cash_flow = None if np.isnan(ocf) else ocf
                dy.cash_equivalents = None if np.isnan(cash) else cash
                dy.total_debt = None if debt <= 0 else debt
                dy.operating_profit = None if np.isnan(op) else op
                dy.depreciation_amortization = None if np.isnan(da_hk) else da_hk
                items.append(dy)
...
def fetch_a(code: str) -> Optional[DupontResult]:
    ...
    try:
        ...
        for yr in years:
            ...
            aa = (ac + ap) / 2 if not (np.isnan(ac) or np.isnan(ap)) else ac
            ae = (ec + ep) / 2 if not (np.isnan(ec) or np.isnan(ep)) else ec
            ocf = _getv_a(cf_y, 'NETCASH_OPERATE', yr)
            cash = _getv_a(bs_y, 'MONETARYFUNDS', yr)
            std = _getv_a(bs_y, 'SHORT_LOAN', yr) or 0
            ltd = _getv_a(bs_y, 'LONG_LOAN', yr) or 0
            debt = (0 if np.isnan(std) else std) + (0 if np.isnan(ltd) else ltd)
            op = _getv_a(inc_y, 'TOTAL_PROFIT', yr)
            da_a1 = _getv_a(cf_y, 'FA_IR_DEPR', yr)
            da_a2 = _getv_a(cf_y, 'IA_AMORTIZE', yr)
            da_a3 = _getv_a(cf_y, 'USERIGHT_ASSET_AMORTIZE', yr)
            da_a = sum(v for v in [da_a1, da_a2, da_a3] if not np.isnan(v))
            if da_a == 0: da_a = np.nan
            if not any(np.isnan(x) for x in [rev, npv, aa, ae]) and rev > 0 and aa > 0 and ae > 0:
                dy = DupontYear(yr, rev, npv, aa, ae, npv/rev, rev/aa, aa/ae, npv/rev * rev/aa * aa/ae)
                dy.operating_cash_flow = None if np.isnan(ocf) else ocf
                dy.cash_equivalents = None if np.isnan(cash) else cash
                dy.total_debt = None if debt <= 0 else debt
                dy.operating_profit = None if np.isnan(op) else op
                dy.depreciation_amortization = None if np.isnan(da_a) else da_a
                items.append(dy)
...
def result_to_dict(res: DupontResult) -> dict:
    return {
        ...
        'valuation': _latest_valuation_data(res),
...
def _latest_valuation_data(res: DupontResult) -> dict:
    """Return the latest year's financial data used for valuation calculations (PS, PCF, EV/EBITDA).
    Values are in yuan (not converted to 亿)."""
    if not res.years:
        return {}
    y = res.years[-1]
    return {
        'revenue': y.revenue,
        'operating_cash_flow': y.operating_cash_flow,
        'total_debt': y.total_debt,
        'cash_equivalents': y.cash_equivalents,
        'operating_profit': y.operating_profit,
        'depreciation_amortization': y.depreciation_amortization,
    }
...
def fetch_kline_a(code: str, start: str = "", end: str = "") -> Optional[list[dict]]:
    code_s = code.replace('.SH', '').replace('.SZ', '').replace('.BJ', '')
    cache_key = f"kline_{code_s}"
    cache_path = os.path.join(CACHE_DIR, cache_key)
    if os.path.exists(cache_path) and time.time() - os.path.getmtime(cache_path) < KL_CACHE_TTL:
        try:
            with open(cache_path, 'rb') as f: return pickle.load(f)
        except: pass
    try:
        prefix = 'sh' if code_s.isdigit() and code_s[0] in '56' else 'sz'
        sym = prefix + code_s
        ...
        try:
            with open(cache_path, 'wb') as f: pickle.dump(records, f)
        except: pass
        return records
    except Exception:
        return None
