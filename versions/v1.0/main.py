...
function renderSingle(s) {
  ...
  html += dupontTable(y);
  if (d.analysis) {
    html += `<div style="margin-top:16px;padding:14px 18px;background:#f8fafc;border-radius:8px;font-size:13px;color:#4a5568;line-height:1.7">${d.analysis.summary}</div>`;
  }
  html += `</div></div>`;
  ...
  if (d.peers && d.peers.length > 0) {
    html += `<div class="card" style="margin-top:16px"><div class="card-header">🏷️ 同类公司参考 <span class="text-muted" style="font-weight:400">点击可加载其杜邦分析</span></div><div class="card-body">`;
...
function renderCompare(ready) {
  let html = `<div class="card"><div class="card-header">📊 多股对比</div><div class="card-body">`;
  html += renderCompareTable(ready);
  html += `<div class="chart-grid">`;
  html += `<div class="chart-box"><canvas id="chartCompRoe"></canvas></div>`;
  html += `<div class="chart-box"><canvas id="chartCompNpm"></canvas></div>`;
...
function makeBar(id, label, labels, data, color, suffix) {
  ...
  if (!ctx) return;
  new Chart(ctx, {type:'bar', data:{labels, datasets:[{label, data, backgroundColor:color+'33', borderColor:color, borderWidth:2,borderRadius:4}]},
...
function makeLine(id, label, labels, data, color, suffix) {
  ...
  new Chart(ctx, {type:'line', data:{labels, datasets:[{label, data, borderColor:color, backgroundColor:color+'20', fill, tension:.3, pointRadius:4, pointBackgroundColor:color, borderWidth:2}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{beginAtZero:true,ticks:{callback:v=>v+suffix}}}}});
}
...
function drawCompareCharts(ready) {
  ...
  ['chartCompRoe','chartCompNpm','chartCompAt','chartCompEm'].forEach((id, i) => {
    ...
    const suffixes = ['%','%','次',''];
    new Chart(ctx, {type:'line', data:{labels:common, datasets},
