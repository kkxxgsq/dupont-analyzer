...
<div class="header">
  <h1>杜邦分析 <span style="font-size:11px;color:#8899b0;font-weight:400">v4.0</span></h1>
...
function loadPeg(code, market) {
  ...
  const eps = latest.net_profit; // in 亿 units per "share unit" - approximated
  const cagr3 = calcCagr(netProfits, 3);
  const cagr5 = calcCagr(netProfits, 5);

  const content = `
    <div style="margin-top:8px;padding:16px 20px;background:linear-gradient(135deg,#f9fafb,#f0f4ff);border:1px solid #dbe4ff;border-radius:10px">
      ...
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:10px">
        <div style="background:#fff;border-radius:8px;padding:10px 14px;text-align:center"><div style="font-size:11px;color:#8899b0">最新净利润</div><div style="font-size:18px;font-weight:700;color:#1a1a2e;margin-top:4px">${latest.net_profit.toFixed(2)}亿</div></div>
        <div style="background:#fff;border-radius:8px;padding:10px 14px;text-align:center"><div style="font-size:11px;color:#8899b0">利润 CAGR(3年)</div><div style="font-size:18px;font-weight:700;color:#1a1a2e;margin-top:4px">${cagr3 !== null ? cagr3.toFixed(1)+'%' : '—'}</div></div>
        <div style="background:#fff;border-radius:8px;padding:10px 14px;text-align:center"><div style="font-size:11px;color:#8899b0">利润 CAGR(5年)</div><div style="font-size:18px;font-weight:700;color:#1a1a2e;margin-top:4px">${cagr5 !== null ? cagr5.toFixed(1)+'%' : '—'}</div></div>
        <div style="background:#fff;border-radius:8px;padding:10px 14px;text-align:center"><div style="font-size:11px;color:#8899b0">年均 ROE</div><div style="font-size:18px;font-weight:700;color:#1a1a2e;margin-top:4px">${(years.reduce((s,y)=>s+y.roe,0)/years.length).toFixed(1)}%</div></div>
      </div>
      <div style="font-size:11px;color:#8899b0;margin-top:6px">PEG 需券商 PE 配合使用。利润 CAGR 显示成长性，ROE 反映质量。成长股 PEG≈1 以下通常更具性价比。</div>
    </div>`;
...
function renderPegButton(code, market) {
  return `<button onclick="loadPeg('${code}','${market}')" style="display:inline-flex;align-items:center;gap:6px;margin-top:10px;background:linear-gradient(135deg,#4a6cf7,#6d8aff);color:#fff;border:none;border-radius:8px;padding:10px 18px;font-size:13px;font-weight:600;cursor:pointer;box-shadow:0 2px 8px rgba(74,108,247,.25);transition:all .2s" onmouseover="this.style.transform='translateY(-1px)';this.style.boxShadow='0 4px 12px rgba(74,108,247,.35)'" onmouseout="this.style.transform='';this.style.boxShadow='0 2px 8px rgba(74,108,247,.25)'">💹 估值参考 <span style="font-size:10px;font-weight:400;opacity:.85"> 即时查看</span></button>`;
}
