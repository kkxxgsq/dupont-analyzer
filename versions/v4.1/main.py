...
function loadPeg(code, market) {
  ...
  box.innerHTML = `
    <div style="margin-top:8px;padding:16px 20px;background:linear-gradient(135deg,#f9fafb,#f0f4ff);border:1px solid #dbe4ff;border-radius:10px">
      ...
      <div style="font-size:11px;color:#8899b0;margin-top:8px">已有数据：净利润 ${npVal}亿 · CAGR(3y) ${cagr3!==null?cagr3.toFixed(1)+'%':'—'} · CAGR(5y) ${cagr5!==null?cagr5.toFixed(1)+'%':'—'} · 年均ROE ${avgRoe}%</div>
    </div>`;
}
...
function renderPegButton(code, market) {
  return `<button onclick="loadPeg('${code}','${market}')" style="display:inline-flex;align-items:center;gap:6px;margin-top:10px;background:linear-gradient(135deg,#4a6cf7,#6d8aff);color:#fff;border:none;border-radius:8px;padding:10px 18px;font-size:13px;font-weight:600;cursor:pointer;box-shadow:0 2px 8px rgba(74,108,247,.25);transition:all .2s" onmouseover="this.style.transform='translateY(-1px)';this.style.boxShadow='0 4px 12px rgba(74,108,247,.35)'" onmouseout="this.style.transform='';this.style.boxShadow='0 2px 8px rgba(74,108,247,.25)'">💹 估值计算器 <span style="font-size:10px;font-weight:400;opacity:.85"> 输入券商数据</span></button>`;
}
